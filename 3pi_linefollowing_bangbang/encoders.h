...
ISR(INT6_vect) {
  ...
  boolean r_B = digitalRead(ENCODER_0_B_PIN);  // normal B state
  boolean r_A = digitalRead(ENCODER_0_A_PIN);  // XOR(AB)
  ...
  state_r = state_r | (r_B << 3);
  state_r = state_r | (r_A << 2);
...
ISR(PCINT0_vect) {
  ...
  boolean l_B = PINE & (1 << PINE2);
  //boolean l_B = PINE & B00000100;  // Does same as above.

  // Standard read from the other pin.
  boolean l_A = digitalRead(ENCODER_1_A_PIN);  // 26 the same as A8
  ...
  state_l = state_l | (l_B << 3);
  state_l = state_l | (l_A << 2);
...
void setupEncoder0() {
  ...
  // initialise the recorded state of r encoder.
  state_r = 0;

  // Get initial state of encoder pins A + B
  boolean r_A = digitalRead(ENCODER_0_A_PIN);
  boolean r_B = digitalRead(ENCODER_0_B_PIN);
  r_A = r_A ^ r_B;

  // Shift values into correct place in state.
  // Bits 1 and 0  are prior states.
  state_r = state_r | (r_B << 1);
  state_r = state_r | (r_A << 0);
...
void setupEncoder1() {
  ...
  // initialise the recorded state of l encoder.
  state_l = 0;
  ...
  // Shift values into correct place in state.
  // Bits 1 and 0  are prior states.
  state_l = state_l | (l_B << 1);
  state_l = state_l | (l_A << 0);
